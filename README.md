...
```bash
docker build --platform linux/amd64 --file Dockerfile --tag analytical-platform.service.justice.gov.uk/ingestion-scanner:local .
```
...
docker run -it --rm \
  ...
  --hostname ingestion-scanner \
  --name analytical-platform-ingestion-scanner \
  analytical-platform.service.justice.gov.uk/ingestion-scanner:local
```
