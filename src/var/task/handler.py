...
def definition_upload():
    try:
        ...
        run_command(f'freshclam --no-warnings --user {user_id} --config-file="{clamav_config}"')

        # Archive the definitions
        run_command('tar --create --gzip --verbose --file=/tmp/clamav/clamav.tar.gz -C /tmp/clamav/database .')
...
def scan(event):
    ...
    s3_client.download_file(landing_bucket_name, object_key, f'/tmp/clamav/scan/{object_name}')

    # Scan the test file
    exit_code, stdout, _ = run_command(f'clamscan --database=/tmp/clamav/database /tmp/clamav/scan/{object_name}')
    print(stdout)
    if exit_code == 0:
...
def move_to_processed(object_key):
    try:
        ...
        s3_client.copy_object(Bucket=processed_bucket_name, CopySource=copy_source, Key=object_key)
...
def move_to_quarantine(object_key):
    try:
        ...
        copy_source = {'Bucket': os.environ['LANDING_BUCKET_NAME'], 'Key': object_key}
        s3_client.copy_object(Bucket=quarantine_bucket_name, CopySource=copy_source, Key=object_key)
        s3_client.delete_object(Bucket=os.environ['LANDING_BUCKET_NAME'], Key=object_key)
